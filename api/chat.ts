...
// --- 通用 API 调用 ---
async function streamApiCall(apiPath: string, payload: any): Promise<Response> {
    try {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    const { text, imageBase64, history, intimacy, userName, currentStep, clickedModule, clickedOption, activeIntent } = req.body;
    ...
    // 【修改点2】如果没有新输入文本，才继续沿用 activeIntent
    else if (activeIntent && !text) {
        triageResult.intent = activeIntent;
        console.log(`[API] 正在进行流程，沿用意图: ${triageResult.intent}, 步骤: ${step}`);
    }
    // 【修改点3】否则，进行意图识别
    else {
